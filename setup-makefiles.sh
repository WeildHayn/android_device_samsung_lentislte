...
LINEEND=" \\"
COUNT=`wc -l ../lentislte/proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../lentislte/proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../lentislte/proprietary-files.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
